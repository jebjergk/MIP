...
-- Backward compat: add column if missing (existing seed unchanged)
...
-- Defaults for AGENT_V0_MORNING_BRIEF (RANKING_FORMULA kept for display; RANKING_FORMULA_TYPE for branch)
...
when not matched then
    insert (AGENT_NAME, MIN_N_SIGNALS, TOP_N_PATTERNS, TOP_N_CANDIDATES, RANKING_FORMULA, RANKING_FORMULA_TYPE, ENABLED)
    values (s.AGENT_NAME, s.MIN_N_SIGNALS, s.TOP_N_PATTERNS, s.TOP_N_CANDIDATES, s.RANKING_FORMULA, s.RANKING_FORMULA_TYPE, s.ENABLED)
when matched then
    update set
