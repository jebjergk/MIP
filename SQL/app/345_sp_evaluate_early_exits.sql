...
-- Single deterministic rule: if any 15-min bar's return since entry
-- reaches PAYOFF_MULTIPLIER x expected_payoff, fire EXIT_SIGNAL immediately.
...
begin
    ...
    for pos in v_positions do
        ...
        begin
            -- Skip if already fired in a prior run (at most one exit per position)
            begin
                select EARLY_EXIT_FIRED
                into :v_prior_fired
            ...
            -- Fail closed: no expected_payoff → no early exit
            ...
            begin
                ...
                into :v_current_price, :v_bar_close_ts,
                     ...
                     :v_first_hit_ts, :v_first_hit_return, :v_first_hit_price
                from (
                    select CLOSE, TS
                ...
                cross join (
                    select
                        ...
                        min(case when (CLOSE - :v_pos_entry_price) / :v_pos_entry_price
                                     >= :v_effective_target
                                 then CLOSE end) as FIRST_HIT_PRICE
                    from MIP.MART.MARKET_BARS
                    where SYMBOL = :v_pos_symbol
                      and MARKET_TYPE = :v_pos_market_type
                      and INTERVAL_MINUTES = :v_interval_minutes
