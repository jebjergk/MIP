...
-- S51-D: Prescreen scores persisted (execution-readiness ranking v2).
SELECT
    'S51-D prescreen scores on latest run' AS CHECK_NAME,
    COUNT(*) AS ELIGIBILITY_ROWS,
    SUM(IFF(e.EVIDENCE_SUMMARY_JSON:combined_rank_score IS NOT NULL, 1, 0)) AS WITH_COMBINED_SCORE,
    SUM(IFF(e.EVIDENCE_SUMMARY_JSON:execution_readiness_score IS NOT NULL, 1, 0)) AS WITH_EXEC_SCORE,
    SUM(IFF(e.EVIDENCE_SUMMARY_JSON:prescreen_rank IS NOT NULL, 1, 0)) AS WITH_RANK
FROM MIP.APP.PROPOSAL_BOARD_RUN r
JOIN MIP.APP.PROPOSAL_BOARD_REVIEW_ELIGIBILITY e ON e.RUN_ID = r.RUN_ID
WHERE r.RUN_ID = (
    SELECT RUN_ID FROM MIP.APP.PROPOSAL_BOARD_RUN ORDER BY STARTED_AT DESC LIMIT 1
);
