...
def get_account_timeline(
    ...
    *,
    ...
    return build_account_timeline(
        run_id,
        attempts.get("simulation_attempt_id"),
        symbol=symbol,
    )


def get_review_chains(run_id: str, *, diagnostic_legacy: bool = False) -> dict:
    state, cfg, _ = _run_state(run_id)
    return list_review_chain_options(run_id, state, cfg, diagnostic_legacy=diagnostic_legacy)


def get_review_catalog(*, run_id: str | None = None, diagnostic_legacy: bool = False) -> dict:
    from .review_catalog import build_review_catalog

    return build_review_catalog(run_id=run_id, diagnostic_legacy=diagnostic_legacy)
...
def get_trade_learning_g1(
    ...
    *,
    ...
    if sym not in [s.upper() for s in symbols]:
        raise HTTPException(status_code=404, detail=f"Symbol {sym} is not in this run.")
    try:
        from .trade_learning_g1 import build_trade_learning_g1_payload, slice_g1_payload_for_replay
        ...
        if replay_through_ts:
            payload = slice_g1_payload_for_replay(payload, replay_through_ts)
    except ValueError as exc:
        raise HTTPException(status_code=400, detail=str(exc)) from exc
    return payload
