...
def get_performance_dashboard_overview(
    ...
    try:
        ...
        snowflake_cost_trend = serialize_rows(
            _safe_rows(
                cur,
                """
                select
        ...
        pw_effectiveness = serialize_row(
            _safe_row(
                ...
                where AS_OF_TS >= dateadd(day, -%s, current_timestamp())
                """,
                (lookback_days,),
            )
        )
        ...
        target_realism = serialize_rows(
            _safe_rows(
                ...
                where DECISION_TS >= dateadd(day, -%s, current_timestamp())
        ...
        for r in snowflake_cost_trend:
            credits = _to_float(r.get("CREDITS_USED")) or 0.0
        ...
        if monthly_cost_rows:
            current_month_snowflake_cost = monthly_cost_rows[-1]["snowflake_cost_usd"]

        total_realized_pnl = _to_float(trade_kpis.get("TOTAL_REALIZED_PNL")) or 0.0
        monthly_total_cost = current_month_snowflake_cost + FIXED_MONTHLY_COST_USD
        win_trades = _to_int(trade_kpis.get("WIN_TRADES"))
        ...
        max_drawdown = _to_float(portfolio_kpis.get("MAX_DRAWDOWN")) or 0.0
        ...
        return {
            ...
            "executive": {
                "kpis": {
                    ...
                    "avg_return_per_trade": _to_float(trade_kpis.get("AVG_PNL_PER_TRADE")) or 0.0,
