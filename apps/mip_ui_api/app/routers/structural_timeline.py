...
def get_available_symbols(market_type: Optional[str] = Query(None)):
    ...
    try:
        ...
        sql = (
            ...
            " TOTAL_SETUPS, ELIGIBLE_SETUPS, PROPOSALS_CREATED,"
            " ACTIVE_PROPOSALS, ACTIONABLE_PROPOSALS, RESEARCH_PROPOSALS,"
            " TRADES_EXECUTED,"
            " DOMINANT_STATE, STRONGEST_FAMILY"
            " FROM MIP.MART.V_STRUCTURAL_TIMELINE_SUMMARY"
            + where
            + " ORDER BY ACTIONABLE_PROPOSALS DESC NULLS LAST,"
              " ACTIVE_PROPOSALS DESC NULLS LAST, PROPOSALS_CREATED DESC NULLS LAST,"
