...
- **Current focus:** Local UX platform — FastAPI + React to surface morning briefs, agent outputs, portfolio KPIs, and training status.
...
| `GET /briefs/latest` | Latest morning brief per portfolio | `MIP.AGENT_OUT.MORNING_BRIEF` |
...
| Page | Purpose |
|------|---------|
| **Home** | Landing and nav to Runs, Portfolios, Audit, Brief, Training. |
...
| **Morning Brief** | Latest brief per portfolio. |
...
- Used by: `EmptyState`, `InfoTooltip`, pages (AuditViewer, Portfolio, MorningBrief) for status badges and glossary tooltips.
...
| [73_UX_RUNBOOK.md](73_UX_RUNBOOK.md) | Inspecting runs, validating briefs, restarting portfolio episodes. |
...
- **Morning brief:** Deterministic and idempotent on `(portfolio_id, as_of_ts, run_id, agent_name)`.
